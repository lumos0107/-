...
import { DUMMY_HISTORY } from '../../../constants/dummy'
import { metersToKm, secondsToMinutes } from '../../../utils/format'

export default function RecordScreen() {
  const totalDist = DUMMY_HISTORY.reduce((s, r) => s + r.totalDistanceMeters, 0)
  const totalTime = DUMMY_HISTORY.reduce((s, r) => s + r.totalTimeSeconds, 0)
  const totalCal = DUMMY_HISTORY.reduce((s, r) => s + r.calories, 0)
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.titleRow}>
          ...
          <View style={styles.countBadge}>
            ...
            <Text style={styles.countValue}>{DUMMY_HISTORY.length}</Text>
          </View>
        </View>

        <View style={styles.summaryCard}>
          <Text style={styles.summaryTitle}>최근 30일</Text>
        ...
        {DUMMY_HISTORY.map((record) => {
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => router.push({ pathname: '/(tabs)/record/[id]', params: { id: record.recordId.toString() } })}
            >
              <View>
